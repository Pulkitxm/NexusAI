...
import { detectCodeInText } from "@/lib/code-detection";
...
export const UserMessage = memo(({ content, className }: UserMessageProps) => {
  ...
  if (codeBlocks.length === 1 && codeBlocks[0].type === "text") {
    return <div className={cn("leading-relaxed break-words whitespace-pre-wrap", className)}>{content}</div>;
  }
        ...
        return (
          <div key={index} className="my-2">
            <SyntaxHighlighter
              ...
              PreTag="div"
              className="!mt-0 !mb-0 overflow-hidden rounded-lg text-sm"
              customStyle={{
                margin: 0,
                borderRadius: "0.5rem",
                fontSize: "0.875rem",

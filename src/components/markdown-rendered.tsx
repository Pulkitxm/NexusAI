      ...
      <ReactMarkdown
        components={{
          ...
          h1: ({ children, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
            <h1 className="text-xl font-bold mb-4 text-slate-800 dark:text-slate-200" {...props}>
              {children}
            </h1>
          ),
          h2: ({ children, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
            <h2 className="text-lg font-semibold mb-3 text-slate-800 dark:text-slate-200" {...props}>
              {children}
            </h2>
          ),
          h3: ({ children, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
            <h3 className="text-base font-medium mb-2 text-slate-800 dark:text-slate-200" {...props}>
              {children}
            </h3>
          ),
          p: ({ children, ...props }: React.HTMLAttributes<HTMLParagraphElement>) => (
            <p className="mb-3 text-slate-700 dark:text-slate-300 leading-relaxed" {...props}>
              {children}
            </p>
          ),
          ul: ({ children, ...props }: React.HTMLAttributes<HTMLUListElement>) => (
            <ul className="list-disc list-inside mb-3 space-y-1 text-slate-700 dark:text-slate-300" {...props}>
              {children}
            </ul>
          ),
          ol: ({ children, ...props }: React.HTMLAttributes<HTMLOListElement>) => (
            <ol className="list-decimal list-inside mb-3 space-y-1 text-slate-700 dark:text-slate-300" {...props}>
              {children}
            </ol>
          ),
